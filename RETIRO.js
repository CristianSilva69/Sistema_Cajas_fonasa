...
// Model for RetiroBrinks entity
    models.RetiroBrinks = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/Sistema_Caja/webresources/modelo.retirobrinks/",
        idAttribute: 'idRetiro',
        defaults: {
            totalEnviado: "",
            totalCheques: "",
            totalBilletes: "",
            totalMonedas: "",
            fechaRetiro: "",
            numeroHojaTrasnporte: "",
            numeroBolsaBillete: "",
            numeroBolsaMoneda: ""
        },
        toViewJson: function () {
            var result = this.toJSON(); // displayName property is used to render item in the list
            result.displayName = this.get('idRetiro');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method == 'create') {
                options.url = 'http://localhost:8080/Sistema_Caja/webresources/modelo.retirobrinks/';
            }
    ...
    // Collection class for RetiroBrinks entities
    models.RetiroBrinksCollection = Backbone.Collection.extend({
        model: models.RetiroBrinks,
        url: "http://localhost:8080/Sistema_Caja/webresources/modelo.retirobrinks/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.RetiroBrinksCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-retirobrinks-list-itemis template identifier for item
                        templateName: '#tpl-retirobrinks-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.retirobrinks = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.retirobrinks,
                    // tpl-retirobrinks-details is template identifier for chosen model element
                    templateName: '#tpl-retirobrinks-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.RetiroBrinks();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-retirobrinks-details is a template identifier for chosen model element
                templateName: '#tpl-retirobrinks-details',
        ...
        getData: function () {
            return {
                idRetiro: $('#idRetiro').val(),
                totalEnviado: $('#totalEnviado').val(),
                totalCheques: $('#totalCheques').val(),
                totalBilletes: $('#totalBilletes').val(),
                totalMonedas: $('#totalMonedas').val(),
                fechaRetiro: $('#fechaRetiro').val(),
                numeroHojaTrasnporte: $('#numeroHojaTrasnporte').val(),
                numeroBolsaBillete: $('#numeroBolsaBillete').val(),
                numeroBolsaMoneda: $('#numeroBolsaMoneda').val()
            };
        }
    });
    new AppRouter();
